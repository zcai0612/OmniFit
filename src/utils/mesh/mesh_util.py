...
def part_removal(full_mesh, part_mesh, thres, device, smpl_obj, region, clean=True):
    ...
    from thirdparties.dataset.PointFeat import ECON_PointFeat
...
def remove_outliers_statistical(vertices, faces, k=20, std_multiplier=2.0, return_trimesh=False):
    """
    使用统计方法移除mesh中的离群点
    
    Args:
        vertices (np.ndarray): 顶点坐标 [N, 3]
        faces (np.ndarray): 面索引 [F, 3]
        k (int): 用于计算邻近点的数量
        std_multiplier (float): 标准差的倍数，用于确定离群点阈值
    
    Returns:
        tuple: (clean_vertices, clean_faces)
    """
    try:
        from sklearn.neighbors import NearestNeighbors
    except ImportError:
        print("警告: scikit-learn未安装，无法使用离群点去除功能")
        return vertices, faces
    ...
    # 计算每个顶点到其k个最近邻的平均距离
    nbrs = NearestNeighbors(n_neighbors=k+1, algorithm='ball_tree').fit(vertices_np)
    ...
    # 标记非离群点
    valid_vertex_mask = mean_distances <= threshold
    
    # 移除无效顶点对应的面
    valid_faces_mask = valid_vertex_mask[faces_np].all(axis=1)
    clean_faces = faces_np[valid_faces_mask]
    
    # 重新索引顶点
    vertex_mapping = np.cumsum(valid_vertex_mask) - 1
    vertex_mapping[~valid_vertex_mask] = -1
    
    clean_vertices = vertices_np[valid_vertex_mask]
    clean_faces = vertex_mapping[clean_faces]
    
    # 移除未被引用的顶点
    mesh_clean = trimesh.Trimesh(vertices=clean_vertices, faces=clean_faces, process=False)
    mesh_clean.remove_unreferenced_vertices()
    if return_trimesh:
        return mesh_clean
    else:
        return mesh_clean.vertices, mesh_clean.faces
...
def center_vertices(vertices, return_params=False):
    if isinstance(vertices, torch.Tensor): 
        vmax = vertices.max(0)[0]
        vmin = vertices.min(0)[0] 
    elif isinstance(vertices, np.ndarray): 
...
def scale_vertices(vertices, bound=1.0, return_params=False):
    if isinstance(vertices, torch.Tensor): 
        vmax = vertices.max(0)[0]
        vmin = vertices.min(0)[0] 
    elif isinstance(vertices, np.ndarray): 
