...
class PointTransformer(nn.Module):
    ...
    def forward(self, pts, mix=False):
        ...
        # torch.save(
        #     neighborhood.detach().cpu(), 
        #     "outputs_vis/0000_00006_02_00021/neighborhood.pt"
        # )
        # torch.save(
        #     center.detach().cpu(), 
        #     "outputs_vis/0000_00006_02_00021/center.pt"
        # )
...
class ScalePointTransformer(nn.Module):
    def __init__(
        self, 
        trans_dim=516, 
        depth=16, 
        ...
        **kwargs
        ...
        self.scale_token = nn.Parameter(torch.zeros(1, 1, self.trans_dim))
        self.scale_pos = nn.Parameter(torch.randn(1, 1, self.trans_dim))
    ...
    def forward(self, pts, mix=False):
        ...
        group_input_tokens = self.reduce_dim(group_input_tokens)
        # prepare scale
        scale_tokens = self.scale_token.expand(group_input_tokens.size(0), -1, -1)
        scale_pos = self.scale_pos.expand(group_input_tokens.size(0), -1, -1)
        # add pos embedding
        pos = self.pos_embed(center)
        # final input
        x = torch.cat((scale_tokens, group_input_tokens), dim=1)
        pos = torch.cat((scale_pos, pos), dim=1)
    
        x = self.blocks(x, pos)
        x = self.norm(x)
        # print(group_input_tokens.shape, scale_tokens.shape, scale_pos.shape, x.shape)
        # concat_f = torch.cat([x[:, 0], x[:, 1:].max(1)[0]], dim=-1)
        scale_tokens = x[:, :1, :]
